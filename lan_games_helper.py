...
class OfflineLanGamesHelper:
    ...
    def build_ui(self) -> None:
        ...
        self.text_size_minus_button = ttk.Button(self.text_size_frame, text="A-", width=3, command=self.decrease_text_size)
        self.text_size_minus_button.grid(row=0, column=0, sticky="ew", padx=(0, 4))
        self.text_size_status_var = tk.StringVar(value=f"Text: {self.ui_scale}")
        self.text_size_status_label = ttk.Label(self.text_size_frame, textvariable=self.text_size_status_var, style="Muted.TLabel")
    ...
    def add_home_tab(self) -> None:
        ...
        self.home_text = tk.Text(home_body, wrap=tk.WORD, height=16)
    ...
    def add_games_tab(self) -> None:
        ...
        self.game_loading_frame.grid_columnconfigure(1, weight=1)
        self.game_loading_label = ttk.Label(self.game_loading_frame, text=self.tr("loading_game_details"), style="Muted.TLabel")
        self.game_loading_label.grid(row=0, column=0, sticky="w", padx=(0, 8))
        self.game_loading_bar = ttk.Progressbar(self.game_loading_frame, mode="indeterminate")
        self.game_loading_bar.grid(row=0, column=1, sticky="ew")
        self.game_loading_frame.grid_remove()
    ...
    def show_settings(self) -> None:
        ...
        self.add_settings_tab(parent=window)
        self.log("Opened Settings in separate window.")
    ...
    def decrease_text_size(self) -> None:
        values = list(UI_SCALE_VALUES)
        index = values.index(self.ui_scale) if self.ui_scale in values else values.index("100%")
        self.set_ui_scale_value(values[max(0, index - 1)])

    def increase_text_size(self) -> None:
        values = list(UI_SCALE_VALUES)
        index = values.index(self.ui_scale) if self.ui_scale in values else values.index("100%")
        self.set_ui_scale_value(values[min(len(values) - 1, index + 1)])

    def update_text_size_labels(self) -> None:
        if hasattr(self, "text_size_status_var"):
            self.text_size_status_var.set(f"Text: {self.ui_scale}")
        if hasattr(self, "ui_scale_value_label"):
            self.ui_scale_value_label.configure(text=f"Current: {self.ui_scale}")
    ...
    def apply_filter(self, select_first: bool = False) -> None:
        ...
        if select_first:
            ...
            target_name = remembered or previous or ""
        if target_name:
            for i, game in enumerate(self.filtered_games):
                if game["name"] == target_name:
                    self.game_list.selection_set(i)
                    # Keep the selection visible, but do not load details until the user clicks the game.
                    break
        # Do not auto-load full game details on startup/filter refresh.
        # Details are loaded lazily only after the user clicks a game.
        self.current_game = None
    ...
    def on_game_selected(self, _event: tk.Event | None = None) -> None:
        ...
        # This keeps the UI responsive and prevents the selected game click from feeling like a freeze.
        ...
        if self.config.get("remember_last_selected_game", True):
            ...
            save_json_file(CONFIG_FILE, self.config)
        self.show_game_loading(game)
        if self.game_select_job:
            try:
                self.root.after_cancel(self.game_select_job)
            except tk.TclError:
                pass
        # A short delay lets Tkinter draw the progress bar before rendering the selected game's sections.
        self.game_select_job = self.root.after(120, lambda g=game: self.finish_game_selection(g))

    def show_game_loading(self, game: dict[str, Any]) -> None:
        self.game_loading_active = True
        self.games_page_title.configure(text=f"Loading {game['name']}...")
        if hasattr(self, "game_status_var"):
            self.game_status_var.set(self.tr("loading_game_details"))
        if hasattr(self, "game_loading_frame"):
            self.game_loading_label.configure(text=self.tr("loading_game_details"))
            self.game_loading_frame.grid()
            try:
                self.game_loading_bar.start(10)
            except tk.TclError:
                pass
        if hasattr(self, "games_detail_text"):
            self.set_text(self.games_detail_text, f"Loading {game['name']} details...\n\nPlease wait a moment.")
        self.root.update_idletasks()

    def finish_game_selection(self, game: dict[str, Any]) -> None:
        if self.current_game is not game and self.current_game and self.current_game.get("name") != game.get("name"):
            return
        try:
            self.update_all_tabs()
        finally:
            self.hide_game_loading()
        self.log(f"Selected game: {game['name']}")

    def hide_game_loading(self) -> None:
        self.game_loading_active = False
        if hasattr(self, "game_loading_bar"):
            try:
                self.game_loading_bar.stop()
            except tk.TclError:
                pass
        if hasattr(self, "game_loading_frame"):
            self.game_loading_frame.grid_remove()
    ...
    def update_home_tab(self) -> None:
        ...
        lines = [
            ...
            f"- Join address preview: {join_address}",
            f"- Visible games: {visible_games} / {total_games}",
            f"- Text size: {self.ui_scale}",
            f"- Theme: {self.theme_name}",
            "",
            "QUICK START",
            "1. Open Games and click a game to load only that game’s details.",
            "2. Check the LAN IP shown above.",
            "3. Use Copy Invite or IP:Port to share only the useful join info.",
            "4. Use Settings for language, themes, tool visibility, and text size.",
            "",
            "FAST ACTIONS",
            "- A- / A+ changes text size instantly. The current size is shown next to the buttons.",
            "- When you click a game, a small loading bar appears while that game's info is prepared.",
            "- Game details are cached after the first click, so reopening the same game is faster.",
            "- Installed detection runs only when requested, so the game list should not freeze.",
            "",
            "SAFETY",
