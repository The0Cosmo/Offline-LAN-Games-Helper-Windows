...
Settings now opens in a separate resizable window from the fixed Settings button in the header. It is no longer mixed into the game/tool tab menu.

The game catalog is loaded once at startup and cached in memory. Game details are generated lazily only after you click a game, so the app does not build every tutorial at startup.

Search and filters work from the cached game list. Installed-game detection is optional and should be refreshed manually so the app does not scan aggressively or freeze the interface.

## Expanded Game Catalog

The Windows catalog now includes more safe LAN/offline/private-server games where legitimate LAN, local hosting, direct IP, or official dedicated server support exists. Online-only, matchmaking-only, or bypass-required games remain excluded.


## Readable Text Controls

The header includes A- and A+ buttons to resize the app text quickly. The current text size percentage is shown beside the buttons, and the same setting is available in Settings.

## Improved Home Page

The Home page now shows a session summary with the selected game, LAN IP, join-address preview, visible game count, current theme, and current text size. Long Home content is scrollable.

## Game Detail Loading Bar
